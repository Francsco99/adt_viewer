...
// Props for the TreePathVisualizer component
interface TreePathVisualizerProps {
  data: {
...
export const TreePathVisualizer: React.FC<TreePathVisualizerProps> = ({
  data,
  activeNodes = [],
}) => {
  ...
  useEffect(() => {
    ...
    treeLayout(root); // Apply layout to hierarchical data
     ...
     // Filter links to include only those between active nodes
    const filteredLinks = root.links().filter(
      (link) =>
        activeNodes[link.source.data.id] === 1 &&
        activeNodes[link.target.data.id] === 1
    );
    ...
    const nodesGroup = g
      ...
      .on("click", (event, d) => {
        // Check if the node is active before allowing selection
        if (activeNodes[d.data.id] === 1) {
    ...
    nodesGroup
      ...
      .attr("ry", 30)
      .attr("fill", (d, i) => 
        (activeNodes[i] === 1 ? "pink" : "white")) // Active node color
      .attr("stroke", (d) =>
        selectedNodes.some((node) => node.data.id === d.data.id) ? "red" : "transparent"
      )
      .attr("stroke-width", (d) =>
    ...
    nodesGroup
      ...
      .attr("fill", (d,i) =>
      (activeNodes[i] === 1 ? "black": "transparent"))
      .text((d) =>
        d.data.id
    );
